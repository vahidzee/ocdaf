...
def sample_permutation_matrix(Gamma: torch.Tensor, n_samples: int, train: bool = False, 
                              tau: th.Optional[float] = None,
                              n_iter: th.Optional[int] = None) -> torch.Tensor:
    """
    ...
    for i in range(n_samples):
        ...
        if train:
            P = sinkhorn(Gamma + gumbel_noise, tau, n_iter)
        else:
            inp = (Gamma + gumbel_noise)
            mat = inp.detach().cpu().numpy()
            ans_pos = hungarian_algorithm(-mat)
            P = torch.zeros_like(Gamma)
            for i in range(len(ans_pos)):
                P[ans_pos[i][0], ans_pos[i][1]] = 1
