...
def qqplot(x: torch.Tensor, y: torch.Tensor, bins=200, alpha=0.5, x_label="x", y_label="y"):
    ...
    for i in range(d):
        axs[i].hist(x[:, i], bins=bins, alpha=alpha, label=x_label, density=True, color="blue")
        # draw the kdensity plot
        axs[i].hist(y[:, i], bins=bins, alpha=alpha, label=y_label, density=True, color="red")
        axs[i].legend()
    return fig
...
class QQplotCallback(pl.Callback):
    ...
    def on_train_epoch_end(self, trainer: "Trainer", pl_module: "LightningModule") -> None:  # type: ignore
        ...
        trainer.logger.log_image("Data", images=[plot], caption=["qqplot"])
        return return_value
