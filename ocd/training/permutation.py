...
class SoftSort(PermutationLearningModule):
    ...
    def flow_learning_loss(self, batch: torch.Tensor, model: OSlow) -> torch.Tensor:
        permutations = self.sample_hard_permutations(batch.shape[0], uniform=self.uniform).detach()
...
class PermutationMatrixLearningModule(torch.nn.Module, abc.ABC):
    def __init__(self,
                 in_features: int,
                 uniform: bool = False,
                 parameterization_type: Literal['sigmoid', 'vanilla', 'log_sigmoid'] = 'sigmoid'):
        super().__init__()
        self.register_parameter(
...
class SoftSinkhorn(PermutationMatrixLearningModule):
    def __init__(self, in_features: int, temp: float = 0.1, iters: int = 20,
                 uniform: bool = False,
                 parameterization_type: Literal['sigmoid', 'vanilla', 'log_sigmoid'] = 'sigmoid'):
        super().__init__(in_features, uniform, parameterization_type)
        self.temp = temp
        self.iters = iters

    def permutation_learning_loss(self, batch: torch.Tensor, model: OSlow) -> torch.Tensor:
        _, gumbel_noise = self.sample_hard_permutations(batch.shape[0], return_noises=True)
...
class GumbelTopK(PermutationMatrixLearningModule):
    ...
    def _loss(self, model: OSlow, batch: torch.Tensor) -> torch.Tensor:
        ...
        log_probs = torch.cat(log_probs).reshape(n_unique, b_size) # shape: (batch, num_uniques, )
...
class ContrastiveDivergence(PermutationMatrixLearningModule):
    def __init__(self, in_features: int, num_samples: int, chunk_size: Optional[int] = None,
                 uniform: bool = False,
                 parameterization_type: Literal['sigmoid', 'vanilla', 'log_sigmoid'] = 'sigmoid'):
        super().__init__(in_features, uniform, parameterization_type)
        self.num_samples = num_samples
