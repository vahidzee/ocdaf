...
class PermutationLearningModule(torch.nn.Module, abc.ABC):
    ...
    def get_best(self, temperature: float = 1.0):
        permutation_samples = self.sample_hard_permutations(
            100, unique_and_resample=False, gumbel_std=temperature
        ).detach()
...
class PermutationMatrixLearningModule(PermutationLearningModule, abc.ABC):
    def __init__(
        ...
        **kwargs,
        ...
        if set_gamma_uniform:
            ...
            all_perm_mats = []
            for perm in set_gamma_custom:
                all_perm_mats.append(turn_into_matrix(torch.IntTensor(perm)).float())
    ...
    def sample_hard_permutations(
        ...
        if self.fixed:
            return (
                turn_into_matrix(self.fixed_perm).unsqueeze(0).repeat(num_samples, 1, 1)
            )
        else:
            gumbel_noise = sample_gumbel_noise(
                (num_samples, *self.gamma.shape),
...
class PermutationMatrixLearningModuleWithBuffer(PermutationMatrixLearningModule):
    def __init__(
        ...
        **kwargs,
        ...
        self.permutation_buffer = torch.zeros(buffer_size, in_features, in_features).to(
            self.gamma.device
        )
    ...
    def _get_in_buffer_index(
        ...
        self.permutation_buffer = self.permutation_buffer.to(permutations.device)
        ...
        with torch.no_grad():
            ...
            msk = (
                (
                    self.permutation_buffer.reshape(
                        self.permutation_buffer.shape[0], -1
                    )
                    .unsqueeze(0)
                    .long()
                    == permutations.reshape(permutations.shape[0], -1)
                    .unsqueeze(1)
                    .long()
                )
                .all(dim=-1)
    ...
    def update_buffer(
        ...
        with torch.no_grad():
            ...
            new_permutations = self.sample_hard_permutations(
                self.num_samples * self.buffer_update, gumbel_std=temperature
            ).detach()
            ...
            new_unique_perms = new_unique_perms[
                sorted_indices[: min(len(sorted_indices), self.buffer_size)]
            ]

            # (2) go over the entire dataloader to compute the scores for this new set of common permutations
            new_scores = torch.zeros(
                len(new_unique_perms), device=self.gamma.device
            ).float()
            ...
            for x in dloader:
                ...
                L = 0
                R = 0
                for unique_perms_chunk in torch.split(
                    new_unique_perms, self.num_samples
                ):
                    ...
                    log_probs = model.log_prob(
                        x_repeated, perm_mat=new_unique_perms_repeated
                    ).detach()
                    log_probs = log_probs.reshape(len(unique_perms_chunk), x.shape[0])
            ...
            if (idx == -1).any():
                ...
                sorted_indices = torch.argsort(appended_scores, descending=True)
                self.permutation_buffer = appended_permutations[
                    sorted_indices[: self.buffer_size]
                ]
                self.permutation_buffer_scores = appended_scores[
                    sorted_indices[: self.buffer_size]
                ]
...
class GumbelTopK(PermutationMatrixLearningModuleWithBuffer):
    def permutation_learning_loss(
        ...
        if (idx == -1).any():
            ...
            unique_perms_repeated = unique_perms.repeat_interleave(b_size, dim=0)
            # shape: (batch * num_uniques, d)
            batch_repeated = batch.repeat(n_unique, 1)

            log_probs = model.log_prob(batch_repeated, perm_mat=unique_perms_repeated)
    ...
    def permutation_learning_loss(
        ...
        with torch.no_grad():
            ...
            if (idx == -1).any():
                ...
                scores[idx == -1] = all_log_probs.reshape(
                    len(unique_perms[idx == -1]), -1
                ).mean(dim=-1)
