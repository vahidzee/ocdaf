...
class SCM:
    ...
    def simulate(self, n_samples, seed=None,
                 ...
                 intervention_function: th.Optional[th.Callable] = None) -> OCDDataset:
        vals = {x: [] for x in self.dag.nodes}
        for sample_i in range(n_samples):
            for v in self.topological_order:
                # sample exogenous noise
                noise = self.get_exogenous_noise(seed + sample_i,
                                                 **self.functional_parameters[v][0])
                inputs = [noise]
    ...
    def draw(self, with_labels=False):
        ...
        for v in self.topological_order:
            ...
            node_labels[v] = self.get_covariate_from_parents_signature(
                inputs, params) if self.get_covariate_from_parents_signature is not None else None
...
GRAPH_GENERATOR_TYPE = ['erdos_renyi', 'barabasi_albert',
                        'random_dag', 'chain', 'collider', 'full', 'tree', 'fork', 'v_structure', None]
...
class SCMGenerator:
    def __init__(self,
                 ...
                 graph_generator_args: th.Optional[th.Dict[str,
                                                           th.Any]] = None,
    ...
    def generate_dag(self) -> np.array:
        ...
        if self.graph_generator_type == 'erdos_renyi':
            ...
            new_dag = nx.gnp_random_graph(self.graph_generator_args['n'],
                                          self.graph_generator_args['p'],
                                          seed=new_seed)
        elif self.graph_generator_type == 'barabasi_albert':
            # generate a random graph using barabasi albert model
            new_dag = nx.barabasi_albert_graph(self.graph_generator_args['n'],
                                               self.graph_generator_args['m'],
                                               seed=new_seed)
        elif self.graph_generator_type == 'random_dag':
            # generate a random graph
            new_dag = nx.gnm_random_graph(self.graph_generator_args['n'],
                                          self.graph_generator_args['m'],
                                          seed=new_seed)
            ...
            if self.graph_generator_type in ['collider', 'v_structure']:
                ...
                new_dag = nx.relabel_nodes(new_dag, {all_nodes[0]: all_nodes[-1],
                                                     all_nodes[-1]: all_nodes[0]})
            ...
            new_dag = nx.random_tree(self.graph_generator_args['n'],
                                     seed=new_seed)
    ...
    def generate_scm(self) -> SCM:
        ...
        for node in dag.nodes:
            ...
            functional_parameters[node] = [self.generate_noise_functional_parameters(
                dag, node, seed=seed, **self.functional_form_generator_args)]
            seed = self.get_iterative_seed()
            functional_parameters[node].append(self.generate_node_functional_parameters(
                dag, node, seed=seed,
                **self.functional_form_generator_args))
        ...
        return SCM(dag, functional_parameters,
