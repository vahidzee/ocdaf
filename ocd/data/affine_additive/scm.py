...
class AffineAdditiveSCMGenerator(SCMGenerator):
    def __init__(self,
        ...
        def get_noise_parameters(dag, v, seed, **kwargs):
            np.random.seed(seed)
            return dict(noise_mean=np.random.uniform(kwargs['noise_mean_low'], kwargs['noise_mean_high']),
                        noise_std=np.random.uniform(
                            kwargs['noise_std_low'], kwargs['noise_std_high'])
                        )
        ...
        if function_type == 'linear_additive':
            ...
            def get_covariate_from_parents(inputs, params):
                return sum(x * w['weight'] for x, w in zip(inputs, params))

            def get_covariate_from_parents_signature(inputs, params):
                sep = ' + '
                ret = f'{params[0]["weight"]:.2f}*{noise_type}({inputs[0]["noise_mean"]:.2f}, {inputs[0]["noise_std"]:.2f})'
                addition = sep.join(f'{w["weight"]:.2f}*x({i})' for i,
                                    w in zip(inputs[1:], params[1:]))
                if addition:
                    ret += sep + addition
                return ret
        ...
        if noise_type == 'gaussian':
            ...
            raise NotImplementedError(
                "noise_type must be one of ['gaussian', 'laplace']")
