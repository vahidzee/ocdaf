...
def build_het_network(in_dim=1):
    return torch.nn.Sequential(
        torch.nn.Linear(in_dim, 100), torch.nn.Tanh(), torch.nn.Linear(100, 2), HetSpindlyHead()
    )
...
def het_fit_nn(x, y, n_steps=None, seed=711, device="cpu"):
    ...
    loader = TensorDataLoader(x.reshape(-1, 1).to(device), y.flatten().to(device), batch_size=len(x))
    set_seed(seed)
    model, losses, _, _, _ = map_optimization(
        build_het_network().to(device).double(), loader, likelihood="heteroscedastic_regression", **map_kwargs
    )
